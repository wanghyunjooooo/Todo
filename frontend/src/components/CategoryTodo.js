...
function CategoryTodo({ categoryId, onDataUpdated }) {
    ...
    useEffect(() => {
        const fetchCategoryTasks = async () => {
            try {
                ...
                console.log("카테고리별 할 일 응답:", res.data);

                if (res.data && Array.isArray(res.data.tasks)) {
                    const grouped = [
                        {
                            categoryName: res.data.category_name || "미분류",
    ...
    if (!tasksByCategory.length) return <div className="no-task-text">이 카테고리에 할 일이 없습니다.</div>;
                            ...
                            {task.memo && (
                                <div className="task-memo-content">
                                    <svg className="task-time-icon" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                                        ...
                                        />
                                        <path d="M8 10H12M8 12H12" stroke="#595959" strokeLinecap="round" strokeLinejoin="round" />
                                    </svg>
