...
function TaskOptionsPopup({ taskId, userId, onClose, onDelete, onEditConfirm }) {
    ...
    const handleCreateRoutine = async () => {
        ...
        try {
            const res = await createRoutine(taskId, selectedRepeatOption, periodStart.toISOString().split("T")[0], periodEnd.toISOString().split("T")[0], userId);
                    ...
                    <div className="editor-box" onClick={(e) => e.stopPropagation()}>
                        <div className="rename-box">
                            <div className="category-list">
                                ...
                                {/* 반복 주기 선택 */}
                                <div className="category-item" onClick={() => setRepeatOptionsVisible(!repeatOptionsVisible)} style={{ cursor: "pointer" }}>
                                ...
                                {periodVisible && (
                                    <div style={{ display: "flex", gap: "8px", marginTop: "8px", flexWrap: "wrap", maxWidth: "100%" }}>
                                        <DatePicker selected={periodStart} onChange={(date) => setPeriodStart(date)} selectsStart startDate={periodStart} endDate={periodEnd} dateFormat="yyyy/MM/dd" popperModifiers={[{ name: "preventOverflow", options: { tether: false, altAxis: true } }]} withPortal={isMobile} />
                                        <DatePicker selected={periodEnd} onChange={(date) => setPeriodEnd(date)} selectsEnd startDate={periodStart} endDate={periodEnd} dateFormat="yyyy/MM/dd" popperModifiers={[{ name: "preventOverflow", options: { tether: false, altAxis: true } }]} withPortal={isMobile} />
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="editor-box" onClick={(e) => e.stopPropagation()}>
                        <div className="rename-box" style={{ width: "350px", padding: "20px" }}>
                            ...
                            <div style={{ maxWidth: "100%" }}>
                                <DatePicker selected={alarmDate} onChange={(date) => setAlarmDate(date)} showTimeSelect dateFormat="yyyy/MM/dd HH:mm" popperPlacement="bottom-start" popperModifiers={[{ name: "preventOverflow", options: { tether: false, altAxis: true } }]} withPortal={isMobile} />
                            </div>
