...
export default function BottomTaskInput({
    ...
    const handleEnter = async (e) => {
        ...
        if (onAddTask) {
            await onAddTask(selectedCategory?.category_id ?? null, bottomInput.trim());
            setBottomInput("");
        }
    };
                ...
                <div className="bottom-content">
                    ...
                    {!hideCategorySelector && (
                        <button className="bottom-menu-btn" onClick={() => setPopupOpen((prev) => !prev)}>
                            ...
                            <span className="bottom-category-span">{selectedCategory ? selectedCategory.category_name : "작업"}</span>
                        </button>
                    )}
                </div>
