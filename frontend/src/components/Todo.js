...
function Todo({ tasksByDate, selectedDate, focusedTaskId, onDataUpdated, categories = [] }) {
    ...
    const handleDeleteTask = async (catIdx, taskIdx) => {
        ...
        setTasksByCategory((prev) => prev.map((cat, idx) => (idx !== catIdx ? cat : { ...cat, tasks: cat.tasks.filter((_, i) => i !== taskIdx) })));
    ...
    return (
        <div className="todo-container">
            ...
            <div className="bottom-input-wrapper">
                <input ref={bottomInputRef} type="text" value={bottomInput} onChange={(e) => setBottomInput(e.target.value)} onKeyDown={handleBottomInputEnter} placeholder="할 일을 입력하고 Enter를 눌러 추가" className="bottom-input" />
            </div>
        </div>
    );
}
